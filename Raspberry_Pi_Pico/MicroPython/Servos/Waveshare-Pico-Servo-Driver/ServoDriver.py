...
# Uses the PIO state machines to drive up to 8 servos.
# Based on Kitronik-Pico-Simply-Servos-MicroPython (https://github.com/KitronikLtd/Kitronik-Pico-Simply-Servos-MicroPython)
...
class WavesharePicoServoDriver:
    # Servo 0 degrees -> pulse of 0.5ms (or 500us), 180 degrees 2.5ms (or 2500 us)
    ...
    minServoPulse = 500
    pulseTrain = 20000
    degreesToUS = 2000/180  # Degrees to microseconds conversion
    piEstimate = 3.1416
    
    # This code drives a PWM on the PIO. It is running at 2 MHz, which gives the PWM a 1uS resolution. 
    ...
    # This is where the looping work is done. The overall loop rate is 1Mhz (clock is 2Mhz - we have 2 instructions to do)    
        ...
        nop()         .side(1)
        label("skip")
        jmp(y_dec, "loop") # Count down y by 1 and jump to PWM loop. When y is 0 we will go back to the 'pull' command
    ...
    # Defaults to the standard pins (2 to 9) and freq for the Kitronik board, but could be overridden
    def __init__(self, numberOfServos = 8):
        servoPins = [0,1,2,3,4,5,6,7]
        self.servos = []
